...
// runners, cmux workspaces, dev servers. Provably dead = no live process AND no heartbeat AND no
// owning session; NOTHING here kills a process (cleanDead only prunes tracking rows).
// Contract-frozen: every export is fail-silent and every subprocess has a ≤2s timeout.
...
// Live crew-runner processes → [{pid,agent,dir,model}]. dir is the LAST argv, so the regex anchors
// on end-of-string; project match is EXACT (a prefix test let …/proj reap …/proj2).
