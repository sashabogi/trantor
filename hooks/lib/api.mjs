...
// (TDD §8), wrapping lib/signed-fetch.mjs (#3916) with hub URL, identity and keypair resolution.
// FAIL-OPEN IS A CONTRACT (acceptance §9 #10): getJSON/signedPost never throw; they resolve to
// { ok:false, status:0, json:null }. No key → unsigned, and the HUB decides under RELAY_AUTH.
...
// Hub URL, PER-PROJECT (TDD §12.1): env RELAY_URL, then config `hubs[project]`, then the legacy
// global `url`, then the local default. Never throws.
...
// THE PROJECT A REQUEST IS ABOUT travels with the request (explicit > payload > query > cwd): the
// hook process's cwd once picked the wrong hub for every card of a session started from ~/development.
...
// identity is born; stamped per session name. Must precede a write under enforce; fail-open.
...
// Reads stay unsigned on purpose: signing /peers and /catchup scopes them to the reader's own
// project and kills cross-project discovery. Reads that need enforce-mode use signedGet below.
/**
 * The shape a fail-open read returns with no answer. `status` stays 0; `timedOut` and `reason`
 * are additive, because a timeout and an outage need opposite responses (#7037).
 */
...
// Signed GET → { ok, status, json|null }. Never throws. For reads that MUST work under
// RELAY_AUTH=enforce (project-scoped reads like /overseer/context); roster reads stay on getJSON.
