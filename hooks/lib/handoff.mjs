// trantor handoff core, shared by PreCompact (at the wall) and the heartbeat (early warning):
// reads live context occupancy, builds a whole-session summary, writes the record, spawns a fresh
// session that takes it over. Both paths share a per-session guard. Seams: docs/CONTRACT-hooks.md.
...
// The #5572 poison guard, from the SAME fixture manifest as the desktop gauge's ContextGuard: report
// the last row unless it falls below 40% of the session max, then the recent maximum, unless the
// last FIVE rows all sit below the floor (a sustained new level is reality).
...
// The transcript logs the model WITHOUT the [1m] marker. Fable is the known 1M exception (#5503);
// an explicit declaration (RELAY_CONTEXT_WINDOW / config.contextWindow) always wins. 0 = unknown.
...
// ---- in-flight guard: true when a sub-agent transcript was written within `withinMs`; the auto
// baton DEFERS on it. Best-effort; false on any error.
// ---- ARMING: the threshold ARMS and the Stop hook FIRES, because PostToolUse only ever runs
// mid-turn. One resolver for the marker path, used by both hooks.
...
// Two signals: subagentsActive() (widening its window only ever defers, the safe direction) and
// the transcript TAIL: only a text-only assistant row reads as idle, the state the Stop hook
// fires on. Every WRITE+SPAWN path asks this; only a typed command or --force bypasses it.
...
export function lastRowMidTurn(transcriptPath) {
  try {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
      ...
      // #6528 follow-up: a trailing user row of ANY kind means in flight. CC does not flush the
      // assistant turn until it ends, so the assistant row's absence is not idle evidence.
      return true;
    }
    return false;
  } catch { return false; }
...
// CC registers live sessions in ~/.claude/sessions/<pid>.json. A session with no live process IS
// at its boundary. "dead" needs another LIVE entry on purpose: a CC too old to keep the registry
// must not turn every mid-turn handoff into an immediate write. "unknown" = say nothing.
...
// Where scrooge actually lives: `command -v` misses ~/.local/bin on a hook's PATH, so the automatic
// paths silently dumped raw transcript. Resolve to an absolute path and exec THAT.
const SCROOGE_DIRS = [
...
// The inline summary is the RECAP, not the record, and an oversized injection is paid for twice.
// Cap at ~4KB keeping BOTH ends (framing + current state) on paragraph boundaries, with an
...
// Trantor State: the structured field on the record (TDD §4.5). `summary` is unchanged; `state`
// rides beside it, validated on write and NEVER capped (#6528: capSummary once ate the STATE
...
// ── #7037: one SIGNED, synchronous hub call whose error cannot be mistaken for data ──────
// A 401 body is valid JSON, so `JSON.parse(out).tasks || []` once read a refusal as an empty
// board. Returns { ok, status, json, reason }, never a bare list: a caller must look at `ok`.
// Synchronous because this whole path is; signedHeaders is synchronous, only transport differs.
...
/**
 * Which card this handoff belongs to: TRANTOR_CARD wins, else this seat's newest open card from
 * /catchup (not /tasks, #7037) on a 2s budget; a truncated bucket reports UNKNOWN, never 0.
 */
...
/**
 * Attach the structured working state: the sidecar when one exists, else derived from git + the
 * model's STATE block. Invalid state attaches null and logs; it NEVER blocks a handoff.
 */
...
// The newest unconsumed MODEL-authored handoff for this project, if still fresh. The digest is the
// fallback, never the replacement (#5648: an automatic digest once superseded the author's words).
...
export function writeHandoff({ projectDir, sessionId, transcript, trigger, summary, force = false, projectName: projectNameArg }) {
  ...
  // Server-side storm guard: sessions on OLD hooks re-fire context-warn handoffs every few minutes.
  // Ask the hub for clearance; a non-forced handoff inside the cooldown is skipped. Manual and
  // at-wall handoffs force through. Fail-open when the hub is unreachable.
  if (!force) {
  ...
  // Open verification gates for this project MUST survive the handoff (a narrative line gets
  // skimmed past). Best-effort, never blocks. #7037: signed, and an unreadable list is reported
  // rather than rendered as "no open gates".
  let verifyGates = [];
  {
...
export function terminalWindowForTty(tty) {
  ...
  // Pass the MULTI-LINE script via stdin, NOT `-e`: a single -e arg keeps the newlines literal and
  // osascript dies, silently returning "" so callers fall through to the WRONG front window.
...
export function armBatonClose(handoffFile, originalWindowId, originalTty, conf = readConfig(), { auto = false } = {}) {
  try {
    ...
    // SAFETY: an AUTOMATIC baton must NEVER close the original session (it once SIGKILLed in-flight
    // work). Auto-close is opt-in (config.autoCloseOriginal:true); a manual /trantor:handoff still
    // closes, non-destructively (baton-close never SIGKILLs and aborts if the original is busy).
...
// The self-announcing fresh session command (single-quoted so it survives osascript→shell).
// Brevity is part of the prompt: a takeover that answers with status dumps loses the operator.
...
// Spawn a fresh self-announcing session WITHOUT the dialog (manual handoff).
// ONE suppression check for every path that opens a window: two names existed and a drill that
// set the wrong one opened live sessions in deleted temp dirs, so both are honoured everywhere.
export function spawnSuppressed() {
  return process.env.TRANTOR_NO_HANDOFF_SPAWN === "1" || process.env.TRANTOR_NO_BATON_SPAWN === "1";
}

// Does this project have a hosted orchestrator pane? Then the PANE is the successor surface
// (#5509 W1): `trantor open` claims the handoff there, and a Terminal window would land on the
// surface the operator is leaving. A stale row costs a skipped window, never a lost handoff.
...
// A session's OWN env knows where it lives (HERDR_PANE_ID) and the project's REGISTRATION knows
// its name long before cwd is worth consulting; basename(cwd) once picked a stranger's window.
// One resolver, shared by write-handoff.mjs --baton AND bin/baton.mjs.
...
// The one resolver. Returns { project, projectDir, pane, surface }: pane = own pane id or "";
// surface = "pane" (must take the pane leg) or "window"; project = the REGISTERED name.
// #6218: the badge wins only where it is TRUE, i.e. cwd lies inside the named project or one of
// its worktrees; a badged shell elsewhere resolves from cwd and ONE warning line names both.
...
// Resolve the ORIGINAL window (id + tty) to close on takeover: controlling tty first, else the
// CURRENT front window. MUST run BEFORE spawning the fresh session, which becomes frontmost.
...
// that survives this session's death. No window machinery here: there is no window to close.
...
// MANUAL one-command baton: spawn the fresh session + arm the close of THIS window. Returns
// { spawned, armed, windowId }. ORDER IS LOAD-BEARING: resolve the original window BEFORE
// spawning, or the successor closes ITSELF. The seams exist so the ordering is drill-tested.
export function spawnBaton({ projectDir, handoffFile, conf = readConfig(),
  ...
  // A DRILL MUST BE ABLE TO SAY NO: a spawn path without an off switch cannot be tested honestly
  // (it once opened real sessions in deleted temp dirs). Reads the REAL env AND the injected one,
  // so a drill under a runner's TRANTOR_NO_*_SPAWN can still exercise each branch.
  ...
  // #6074, checked FIRST: HERDR_PANE_ID means the pane leg keyed by THAT pane, regardless of cwd,
  // and the whole window machinery is forbidden (a pane session owns no Terminal window).
