#!/usr/bin/env node
// USAGE v2: the Claude statusline sidechannel (docs/RESEARCH-orca-usage.md §1.1). CC >=2.1.80 pipes
// a JSON blob with `rate_limits` into the statusLine command every turn. This forwarder POSTs the
// windows to /usage/claude (signed) and prints NOTHING: tee it ahead of the real statusline.
// Every failure is swallowed. Floor: one POST per session per 15s (the hub dedupes inside 30s).
