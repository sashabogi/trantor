#!/usr/bin/env node
// trantor Notification hook: card CC's own background agents (fork/subtask sessions that never
// pass SubagentStart/Stop) as source "cc-bg-agent", a distinct population from cc-subagent.
// agent_needs_input → blocked, agent_completed → done. The agent-id field name is unverified
// live: several candidates are read; TRANTOR_DEBUG_NOTIFY=1 dumps the raw payload. Fail-silent.
