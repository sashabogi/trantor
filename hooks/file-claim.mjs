#!/usr/bin/env node
// trantor PreToolUse file-claim: before every edit the session posts a claim and the hub answers
// with any LIVE claim on the same file by another session, handed to the model as context.
// Informational, never blocking (a fail-open lock is worse than none); fail-open and cheap: a
// per-(session,file) stamp throttles re-claims, the FIRST touch always goes out. CONTRACT-hooks.md.
...
try {
  ...
  // NO permissionDecision on purpose: this hook only WARNS, and additionalContext reaches the model
  // on its own. Returning "allow" would approve the call and bypass the operator's own rules.
