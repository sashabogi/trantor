...
// card so a regular session's work shows IN PROGRESS on the board. Trivial acks don't refocus.
// NO LLM call on the turn path: a heuristic title posts now, bin/focus-title.mjs rewrites it
// DETACHED. Never blocks or delays the turn.
...
// §5 recap net (SYSTEM-CONTRACT): while a claimed handoff is unrecapped, EVERY prompt before the
// first Stop carries the reminder; stop-inbox clears the stamp at the first turn boundary.
