...
async function main() {
  ...
  // Identity EXACTLY as mcp.mjs/heartbeat resolve it: resolveProject keys off the GIT ROOT and hostId is
  // the stable machine id (hostname() drifts to ".local" on some networks and splits one machine into two peers).
