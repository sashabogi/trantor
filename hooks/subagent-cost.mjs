#!/usr/bin/env node
// trantor SubagentStop hook: when a CC sub-agent finishes, read ITS OWN transcript's usage and post
// a board card with the NOTIONAL API cost (plan-covered, not billed). This is the orchestrator's
// own-work blind spot that crew and Scrooge don't cover. Fail-silent.
