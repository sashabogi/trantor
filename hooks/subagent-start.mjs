#!/usr/bin/env node
// trantor sub-agent in-flight hook — posts a "doing" card so sub-agent work shows IN PROGRESS; the
// SubagentStop hook flips it to "done". ONE script, TWO events: PreToolUse (Task|Agent) has the prompt
// and CREATES the card; SubagentStart has the real agent_id + parent and ENRICHES it (or creates one
// keyed by agent_id if none exists). Registering both never double-posts. Fail-silent throughout.
