#!/usr/bin/env node
// trantor Stop hook: don't go idle with a peer waiting on you. Pairs with inbox-deliver (mid-turn).
// Delivery goes through the receiving agent's OWN harness, never by driving its terminal (the
// removed approach could aim keystrokes at a bypass-permissions agent). A Stop hook that prints
// {decision:"block",reason} makes the model continue. Rules: docs/CONTRACT-hooks.md.
...
// An empty inbox is not nothing outstanding: ask the hub what this session is owed and refuse ONCE
// if any of it went quiet. Only `stalled` blocks (a healthy open contract is just in progress);
// `abandoned` never blocks (it was `stalled` long enough to have been reported). Fail-open.
const OVERDUE_MS = (() => {
...
async function main() {
  ...
  // A Stop IS the turn boundary: a baton armed mid-turn fires here, detached and never awaited,
  // with the arming cleared FIRST so a crashed worker cannot re-fire on every Stop.
  // #6528: a boundary is only a boundary when NOTHING is still running; with a sub-agent in flight
  // the arm STAYS and the heartbeat's hard cap keeps a never-idle session from being armed forever.
  try {
