...
console.log("\nA live question from a live seat is NOT stale:");
{
  ...
  // Code only — the comment above stalenessOf explains WHY presence was withdrawn and rightly
  // names lastSeen, so a naive text search would fail on the explanation itself.
  const code = readFileSync(join(ROOT, "desktop/src/features/inbox/staleness.ts"), "utf8")
    .split("\n").filter(l => !l.trim().startsWith("//") && !l.trim().startsWith("*")).join("\n");
  ok("…and the module no longer consults presence at all", !/lastSeen/.test(code),
    (code.match(/.*lastSeen.*/) || [""])[0].trim());
}
...
console.log("\nWhether the sender is online is NOT a staleness signal:");
{
  // Tried and withdrawn 2026-08-24. It reported a live duty seat — running six hours, seen by the
  // hub two minutes earlier — as "gone a while", because the view snapshotted peers on mount and
  // compared that to a live clock. The stale data exposed the real error: a message does not stop
  // meaning anything because its author went home.
  const m = msg(12, "glm:proj");
  ok("a long-silent sender does NOT make the message stale",
    stalenessOf(m, [m], [gone("glm:proj")], [], NOW).stale === false);
  ok("a sender absent from the peer list entirely does not either",
    stalenessOf(m, [m], [], [], NOW).stale === false);
  ok("…even with an empty peer list AND a much later clock",
    stalenessOf(m, [m], [], [], NOW + 30 * 24 * 60 * 60 * 1000).stale === false);
}
