...
// (the migration importer, the deploy scripts and hub.mjs consume it simultaneously).
// DESIGN: the EVENT LOG is the table; board state is a PROJECTION of it. Every field below must
// round-trip through Postgres. Invariants and the change protocol: docs/CONTRACT-lib.md.
...
// Keys that MUST round-trip through kv; losing one on restart is a failed migration. `proposals`:
// a denied proposal is a MEMORY the hub refuses re-proposals against. `contractReap`: forgetting it
// re-announces the whole ghost backlog.
...
// INVARIANTS carried forward from 0.17.54 (docs/CONTRACT-lib.md): legacy flat card events with
// legacy type names, new types dotted; messages carry refs[] never task_id; threads are DERIVED;
// delivered_up_to is MONOTONIC; retention is time-based and never deletes a projection row.
...
// {"src":"<writer-id>"}; the hub reloads its projection on a foreign src, or its boot cache
