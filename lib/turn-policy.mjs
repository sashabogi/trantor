// Per-turn policy for a crew seat (#6134): is a bus message worth a turn, which card a session belongs
// to, what a turn cost, when an exhausted seat may wake. Lives outside the runner so it is unit-testable.
...
/// #7061: a citation that HANDS THE CARD OVER, not one that merely mentions it: a work order opens with
/// what shipped, so the earliest id is routinely a DONE card. Shape (an assignment verb or label) decides.
...
/// #7763: the statuses "this card is work someone owes" — relay_board's mine view and the runner's
/// own-card resolution must agree on it, so the set lives in the one file both sides already import.
export const OPEN_CARD_STATUSES = ["doing", "testing", "todo"];

/// #7763/#7765: a MESSAGE-CARD is a transcript of a bus message parked on the board ("NEW BUS
/// MESSAGE for you: …", the join note "You just joined …"), never work. A wake that cites only one
/// of these binds to no card at all — a seat sent chasing one spends the turn on card bookkeeping
/// and never reaches the question (#7765's kimi specimen, 1201s on a phantom id).
export const MESSAGE_CARD_PREFIXES = ["new bus message", "you just joined"];
export function isMessageCardTitle(title) {
  const t = String(title || "").trim().toLowerCase();
  return MESSAGE_CARD_PREFIXES.some((p) => t.startsWith(p));
}
...
/// #7061: the card a wake batch binds to. Direct messages outrank @mentions, the NEWEST assignment wins,
/// and only an assignment-free batch falls back to the newest message's first citation.
...
/// #7060: why THIS turn is not assembled from a WorkingState, or null when it is. The boot line proves
/// configuration only, so the runner asks this every turn; reasons are ordered by which constraint binds.
...
/// #6228: may a wake from `senderProject` reach a seat in `seatProject`? Same project always; a declared
/// `trantor policy link` opens two named projects; anything else is dropped, never worked.
