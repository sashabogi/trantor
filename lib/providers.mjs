// trantor provider registry — ONE truth for "is this provider ready to seat?" (#6390). The JSON row
// shape is FROZEN (docs/CONTRACT-lib.md §providers); every live probe is lib/balances.mjs, not here.
...
// keychain item "Claude Code-credentials": attribute-only lookup, and only when no home was injected.
...
async function buildRow(p, ctx) {
  ...
  // 3) no auth at a KNOWN location → not_logged_in, naming the artifact; a provider whose artifact we
  //    cannot read (agy) gets unknown instead, with the how-to-check in the reason.
...
export async function providerStatus(opts = {}) {
  ...
  const ctx = {
    ...
    env: opts.env ?? resolveKeys(process.env, [join(home, ".token-scrooge", ".env"), join(home, ".agent-bus", ".env")], opts.home ? null : resolveSecrets()),
    path: opts.path,
...
export async function providerVerify(name, candidateKey, opts = {}) {
  ...
  const home = opts.home || homedir();
  const baseEnv = opts.env ?? resolveKeys(process.env, [join(home, ".token-scrooge", ".env"), join(home, ".agent-bus", ".env")], opts.home ? null : resolveSecrets());
