// trantor provider registry: ONE truth for "is this provider ready to seat?" (#6390), Orca's
// onboarding shape: PATH detect, the CLI's own login command and credential artifact, and connected
// = a LIVE usage call succeeded. Row shape FROZEN for #6391/#6392; every row carries a state and a
// non-empty reason. Probes are never duplicated: every live check is a lib/balances.mjs adapter.
...
// Claude Code keeps its OAuth token in ~/.claude/.credentials.json (or $CLAUDE_CONFIG_DIR), then the
// keychain item "Claude Code-credentials". Attribute-only lookup (no -w): a health read must never
// raise a GUI prompt. Keychain only when the caller did not inject a home (drills stay hermetic).
...
export const PROVIDERS = [
  { provider: "claude", label: "Claude", kind: "windows", connect: "cli-login",
    binary: ["claude"], loginCmd: "claude", loginRun: ["claude"], logoutRun: ["claude", "logout"], envKeys: [], probe: "balances",
    auth: claudeAuth,
    hint: "sign in with your Anthropic account on first run" },
  { provider: "codex", label: "Codex", kind: "windows", connect: "cli-login",
    binary: ["codex"], loginCmd: "codex login", loginRun: ["codex", "login"], logoutRun: ["codex", "logout"], envKeys: [], probe: "balances",
...
async function buildRow(p, ctx) {
  ...
  // 3) no auth at a KNOWN location → not_logged_in, naming the artifact and the fix (the probe would
  //    only echo it slower). A provider with no readable artifact AND no probe is unknown, with
  //    the how-to-check in the reason.
