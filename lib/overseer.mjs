...
export function detectCollisions({ peers = [], claims = [], links = [], cards = [], autonomy = {}, now } = {}) {
  ...
  // A link is DECLARED, so two sessions being live on both sides restates the operator and is a
  // permanent STATE (#7029). Only an EVENT is worth a wake: one FILE PATH claimed live from both
  // sides, or one CARD ID held by live sessions from both sides (`workedBy` is the signed evidence).
  // No event, no warning: under-warning is the intended bias.
