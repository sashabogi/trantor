// trantor sub-agent manifest — a READ-TIME projection of a session's sub-agents from on-disk transcripts
// (meta.json, the parent jsonl, each agent jsonl, a disk reconcile), so a successor can re-derive it.
import { readFileSync, readdirSync, existsSync, statSync } from "node:fs";
import { join, dirname, basename, relative } from "node:path";
import { homedir } from "node:os";
import { asString } from "./decode.mjs";
...
function analyzeAgent(meta, completedIds, projectRoot) {
  ...
  for (const r of parseLines(meta.jsonlPath)) {
    ...
    for (const b of c) {
      ...
      if (b?.type === "text") { const text = asString(b.text)?.trim(); if (text) result = text; }
    }
  }
