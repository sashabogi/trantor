// trantor sub-agent manifest: what a session's sub-agents were doing, as a READ-TIME projection of
// the transcripts on disk (a killed build once clobbered a finished implementation and the
// successor rebuilt it believing nothing survived). Four sources under ~/.claude/projects/<proj>/<sid>/:
// subagents/*.meta.json, the parent jsonl's tool_result ids, each agent-*.jsonl, a disk reconcile.
