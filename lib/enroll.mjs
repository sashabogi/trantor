// trantor: make a seat self-enrol on an authenticated hub. A brand-new seat keypair is an unknown
// identity on an enforce hub and fails open into silence. So the seat enrols itself at startup with
// a single-use, project-scoped invite minted by the OPERATOR's owner key (a crew only ever runs on
// the operator's machine); the seat never holds owner rights. docs/CONTRACT-lib.md.
