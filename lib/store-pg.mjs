// trantor Phase 1 Postgres store: implements the frozen STORE_API from ./store-contract.mjs as the durable
// backing when RELAY_STORE=pg. SAFETY: the typeof checks here decode untrusted jsonb rows at the boundary.
...
// The COLUMNS are the truth, so the payload spreads FIRST and columns after: a stored `id` spread last
// once overwrote the row id and silently killed the append-only log (docs/CONTRACT-lib.md §store-pg).
...
export class PgStore {
  ...
  // Incremental persist: touch ONLY rows the hub has seen (diffById), so an external writer survives and
  // the hub stops rewriting every row per tick. Events stay append-only; NOTIFY fires inside the transaction.
