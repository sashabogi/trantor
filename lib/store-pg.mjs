// trantor Phase 1 Postgres store: the frozen STORE_API from ./store-contract.mjs, the durable backing
// when RELAY_STORE=pg while the hub keeps its in-memory projection.
// SAFETY: this module is the Postgres I/O decoder boundary; its runtime `typeof` checks validate
// untrusted jsonb rows and caller-supplied JSON before those values enter the hub domain.
...
// The COLUMNS are the truth; the payload is only the fields with no column. Payload FIRST, columns
// after: a payload `id` spread last once overwrote the row id and silently stalled the append-only log.
...
function msgFromRow(row) {
  ...
  // #7140: the two fields contractIsAck() reads. Absent unless the sender said so, exactly as the
  // hub mints them (`wake` only when false, `kind` only when given). A NULL wake stays absent —
  // never false — so a row written before the column existed remains a normal contract.
  if (row.wake === false) m.wake = false;
  if (row.kind) m.kind = String(row.kind);
  return m;
}

// The message columns the ack disposition depends on, in INSERT order. NULL is "not said".
const msgAckCols = (m) => [m.wake === false ? false : null, m.kind ? String(m.kind).slice(0, 40) : null];
...
export class PgStore {
  ...
  async appendMessage(orgId, msg) {
    ...
    const r = await this.pool.query(
      `WITH next_id AS (SELECT COALESCE(MAX(id),0)+1 AS id FROM messages WHERE org_id=$1)
       INSERT INTO messages(id, org_id, ts, from_session, to_session, project, text, refs, re, wake, kind)
       SELECT id, $1, $2, $3, $4, $5, $6, $7::jsonb, $8, $9, $10 FROM next_id RETURNING id`,
      [orgId, ms(msg.ts), msg.from || msg.from_session || "anon", msg.to || msg.to_session || "all", msg.project || "", String(msg.text ?? ""), JSON.stringify(refs), Number(msg.re) > 0 ? Number(msg.re) : null, ...msgAckCols(msg)],
    );
    return Number(r.rows[0].id);
  }
        ...
        await c.query(
          `INSERT INTO messages(id, org_id, ts, from_session, to_session, project, text, refs, re, wake, kind)
           ...
           ON CONFLICT(org_id, id) DO UPDATE SET ts=EXCLUDED.ts, from_session=EXCLUDED.from_session, to_session=EXCLUDED.to_session,
             project=EXCLUDED.project, text=EXCLUDED.text, refs=EXCLUDED.refs, re=EXCLUDED.re, wake=EXCLUDED.wake, kind=EXCLUDED.kind`,
          [Number(m.id), orgId, ms(m.ts), m.from || "anon", m.to || "all", m.project || "", String(m.text ?? ""), JSON.stringify(Array.isArray(m.refs) ? m.refs : []), Number(m.re) > 0 ? Number(m.re) : null, ...msgAckCols(m)],
        );
      }
      await c.query(
  ...
  // Incremental persist: only rows the hub itself has seen (diffById), so an external writer's rows
  // survive (saveSnapshot rewrote the org wholesale, once per second). Events stay append-only
  // (ON CONFLICT DO NOTHING). NOTIFY on CHANGE_CHANNEL inside the transaction; `src` lets a hub
