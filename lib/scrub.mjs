/* Trantor – lib/scrub.mjs: the event log is append-only, so credential-shaped strings are redacted
 * before they are logged. Pure functions, no I/O, never throws. */
...
const patterns = [
  ...
  // 64-char lowercase hex is an Ed25519 private key AND a pubkey AND a sha256 digest, which the protocol
  // emits constantly, so an INTENT word within the preceding 40 chars is required (`"privkey": "…"`).
