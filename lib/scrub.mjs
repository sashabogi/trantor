/* lib/scrub.mjs: the event log is append-only, so a secret that enters it can never be deleted.
 * Detects and redacts credential-shaped strings before they are logged. Pure, no I/O, never throws. */
...
const patterns = [
  ...
  // 64-char hex is an Ed25519 PRIVATE key, but also a PUBLIC key and a sha256 digest, both of which
  // the protocol emits constantly. So require an INTENT word within the preceding 40 chars, which
  // is how a leak actually looks (`"privkey": "…"`); a bare pubkey or digest passes.
