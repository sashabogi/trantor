...
// ROOT basename (basename(cwd) let a project fragment into lanes); RELAY_PROJECT always wins.
// The hub applies an alias map on top. Seams: docs/CONTRACT-lib.md.
...
export function resolveProject(cwd = process.cwd(), env = process.env) {
  ...
  // ~/.agent-bus/worktrees/<project>/<agent>, so basename named the project after the AGENT).
  // git-common-dir points at the main repo from any worktree; a no-op for normal checkouts.
  if (root) {
    try {
...
// A project lives on exactly ONE hub and codependent projects share it. Order: RELAY_URL →
// config hubs[project] → the legacy global `url` → the local default. Never throws (fail-open).
export const DEFAULT_HUB_URL = "http://127.0.0.1:4477";

// The bus directory, resolved in ONE place. Both override names are honoured (AGENT_BUS_DIR and
// RELAY_DATA_DIR are both in use across the hooks); a reader that honours neither mutates the
// user's REAL state during a drill.
...
// The orchestrator-session map: which claude conversation IS this project's orchestrator thread.
// One TAB-separated row per project, shared by `trantor open`, `trantor adopt` and the hooks, so
// the successor of a handed-off orch thread is followed instead of a dead session id resumed.
...
// Is the session writing a handoff RIGHT NOW the orchestrator thread? Decided by evidence:
// 1. TRANTOR_ORCH matching the project (the pane runs the recorded thread by construction);
// 2. else the recorded thread's transcript freshly WRITTEN (mtime, as `trantor adopt` judges).
// Returns the recorded orch session id when the writer is that thread, else "".
...
// HOW a hub was chosen, not merely which one: the silent fallback to an unread hub is the most
// expensive failure this system has, so provenance is part of the answer and callers can warn.
//   via "env" → RELAY_URL · "pin" → config hubs[project] (the only DELIBERATE routing)
//   via "global" → config url (never pinned) · "default" → built-in local (no config at all)
...
// Is this directory a real project seat, and if not, WHY not? "" for a seat, else a short reason.
// A non-seat must never register: it mints an unpinned phantom board on the local hub while the
// crew lives on the remote one. An explicit RELAY_SESSION/RELAY_PROJECT is the deliberate opt-in.
export function nonSeatReason(dir = process.cwd(), env = process.env) {
  try {
    ...
    // A WORKSPACE CONTAINER (~/development): registers as project "development", which nothing pins.
    // A plain non-git directory is NOT disqualified: plenty of real projects have no repo yet.
...
// Stable machine identity: os.hostname() is network-dependent and forks one machine into two
// peers. Resolved ONCE and persisted to ~/.agent-bus/machine-id: RELAY_HOST_ID > persisted id >
// macOS LocalHostName > hostname() without its domain suffix.
let _hostId = null;
// Wrap a command so it runs with the seat's provider keys loaded, highest-priority file FIRST.
// Each file is PREPENDED, so the one prepended LAST runs FIRST, and in shell the one that runs
// LAST wins: callers pass files in priority order. Getting it backwards once handed every seat
// Scrooge's key; test-crew-env.mjs proves the order against a REAL shell.
