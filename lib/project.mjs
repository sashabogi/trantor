...
// One row per project, TAB-separated — the format `trantor open` (bin/crew.mjs) and `trantor adopt`
// already share. Read/written here too so the hooks can follow the thread across a handoff: the
// successor of a handed-off orch thread gets a NEW session id, and a map nobody updates makes
// `trantor open` resume a dead conversation (the 2026-08-27 seam).
...
// Is the session writing a handoff RIGHT NOW the project's orchestrator thread? The relay MCP
// server has no harness session_id (see mcp.mjs), so this is decided by evidence, not assertion:
//   1. TRANTOR_ORCH matching the project is definitive — `trantor open` badges the pane's claude
//      (the MCP server inherits its env), and the pane runs the recorded thread by construction.
//   2. Otherwise, the recorded thread's transcript freshly WRITTEN: a writer is mid-turn, so its
//      transcript got the tool_use entry seconds ago — the same mtime-as-evidence standard as
//      `trantor adopt`. An idle orch pane's transcript is stale, so a Terminal twin writing a
//      handoff is correctly NOT the orch thread.
