...
// Stable project key for a working directory. RELAY_PROJECT > bus-worktree path > git-root
// basename > cwd basename. The env comes in as a parameter (#6218): the handoff resolver must be
// able to resolve the cwd WITHOUT the shell's RELAY_PROJECT — a badge that lied about the cwd
// must not get a second voice through the fallback's own env read.
