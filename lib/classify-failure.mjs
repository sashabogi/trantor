...
export function verdictFor(realExit, effExit, emptyOutput, ownText, emptyTurn = false) {
  if (realExit === 0 && effExit === 0) return emptyTurn
    ? "classified empty-turn because exit 0 with no worktree change and no substantive output"
...
export function classifyFailure(exit, errText, emptyOutput = false, emptyTurn = false) {
  ...
  // #7759: exit 0 with bytes on the stream but neither a changed worktree nor substantive
  // output is a HOLLOW turn — the CLI printed its banner and quit without doing anything.
  if (emptyTurn) return { reason: "empty-turn", matched: "exit 0 with no worktree change and no substantive output" };
