...
/** #7752: what a CUT turn's exit says about WHY the CLI died. 141 is 128+13 — SIGPIPE: the time
 *  box's sweep ends a CLI still holding the turn's pipes, so the pipe death is the box's
 *  signature, recorded in the ledger row as the cut signal. Any other exit is not a signal. */
export function cutSignalFor(exit) {
  return Number(exit) === 141 ? "SIGPIPE" : "";
}
