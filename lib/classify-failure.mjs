// Failure classification for crew seats (#5868), split out of the runner so the rules are
// unit-testable against the REAL specimens that misfired (the seat's own PROMPT echoed back
// once read as auth death and as exhaustion). Three rules: stripPromptEcho (replay is not
// speech), looksLikeAuthDeath (short error-only output, never after a commit), classifyFailure.
import { readFileSync } from "node:fs";

export const AUTH_MARKER_RE = /unauthor|401|403|forbidden|invalid[ _-]?api[ _-]?key|authentication? failed|token expired/i;

/** Prompt lines (≥40 chars) echoed back by the CLI are replay, not speech. Echoes are rarely
 *  byte-identical, so both sides are normalized (ANSI off, whitespace collapsed) and a long line
 *  is dropped when it CONTAINS a prompt line, IS a fragment of one, or shares a ≥40-char RUN. */
const ECHO_RUN = 40;
export function stripPromptEcho(errText, promptText) {
  ...
  // Whole-prompt normalized text for the short-line VERBATIM check: a short echoed fragment has no
  // 40-char run but still reappears byte-for-byte, and must not trip looksLikeAuthDeath (#6110).
