...
/**
 * Drop escalations whose recipient has ALREADY read the message.
 *
 ...
 * acted on. The escalations were real when duty first saw them; the recipient read them before duty
 * got a turn; nothing re-checked. A nudge WAKES A SESSION and every wake costs a turn on both sides,
 * so nudging for delivered mail is not a harmless retry.
 *
 * `isDelivered` is injected so this is testable without a hub. The runner passes a /peer lookup —
 * note the SINGULAR endpoint: /peers does not serialise deliveredUpTo, a gap that already cost one
 * wrong diagnosis. Unknown is never treated as delivered: if the check cannot answer, the nudge
 * stands, because a missed nudge is worse than a redundant one.
 */
