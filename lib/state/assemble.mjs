...
// Trantor State P5: assemble (TDD §4.6). preamble + STATE_DELIM + renderState + TAIL_DELIM + tail
// + OBS_DELIM + observation. The bytes before STATE_DELIM are the caller's preamble and NOTHING
// else, or provider prefix caching never engages and the cost curve stays O(T);
// test/state/test-assemble.mjs hashes the prefix from two states to hold that invariant.
...
/**
 * Cap a string to ~cap tokens, KEEPING THE END (§4.8): the head of a gate tail or a card log is
 * the stale part.
 */
...
/**
 * The state as the seat reads it: compact, deterministic, bounded by the schema caps; runtime ext
 * keys stay private. Total: a null or shape-broken state renders as "" and never throws.
 */
...
/**
 * One step's prompt. Total and deterministic; every input is capped, so the prompt is O(1) in
 * turn count by construction. @returns {string}
 */
