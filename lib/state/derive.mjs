// Trantor State P4: derive (TDD §4.5). A WorkingState for a handoff with no sidecar, from git and
// the model's own STATE block. A DERIVED STATE CARRIES NO CREDIT: every path is verified:false and
// `verify` is empty, so the successor re-earns its evidence. The model's block goes through
// applyTurn, never a second parser, so the write matrix and caps hold here for free.
 ...
 * so they ride ctx.files into the runtime pass; a patch cannot write `files` at all.
 */
 ...
 * `in_flight`, never `done`: "still open" costs one re-check, "finished" is an unverified claim.
 */
 ...
 * the count, not sent into applyTurn where an overflow rejects the whole patch (§3.1).
 */
...
/**
 * Build a WorkingState for a handoff with no sidecar. A rejected patch does not cost the state.
