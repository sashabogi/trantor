...
// Trantor State P6 — the runner-side driver (TDD §4.1, §4.8, §7.3). It is the ORDER: readState →
// assemble → CLI → tier 1 → applyTurn → commit (CAS) → promote → execute, recorded in `trace`.
...
// The flag name lives in flags.mjs (#7159), beside the resolution list — re-exported so the
// runner's import surface is unchanged.
export { STATE_ENV } from "./flags.mjs";

/** §7.3's window: the rolling invalid rate is measured over a seat's last 20 turns. */
export const BREAKER_WINDOW = 20;

/** The JSON Schema handed to `claude --json-schema` (§4.6), built from LISTS so a fifth list cannot
 *  drift between validator and CLI. `action` is the §4.6 collapse: no `{tool,input}` on a CC seat. */
...
/** Whether the installed CLI can enforce the grammar (§6). No flag = state mode stays OFF: a seat that
 *  cannot be held to the schema must not run on a path whose budget assumes it is.
...
/** Parse the `--output-format json` envelope into the cost struct and the TurnResult. `result` may be an
 *  object or a JSON string; anything else is `turn: null` with a reason, never a guessed empty patch.
...
/** The card log a state step is read against (§4.1's `tail`), decoded from `GET /tasks`: note rows are
 *  a string on older records and `{by, text}` on current ones. "" when the card is not on the board. */
...
/** §7.3's circuit breaker over recorded patch outcomes. Counts MALFORMED only (UNVERIFIED_DONE and
 *  NEEDS_GATE are the system working), and the window must be FULL before it can trip.
...
/** ONE STEP, in the §4.1 order. `deps` injects every side effect so a test can read `trace` back;
 *  executeAction runs LAST, after promote, and only on an accepted patch.
 *  @returns {{ ok: boolean, trace: string[], code: string|null, message: string, step: object, state: object|null,
 *             action: object|null, observation: string, promoted: string|undefined, patchRecord: object }} */
export async function runStep({
  ...
  // Per attempt, not per step (#7226): a cut on the first attempt belongs to that attempt's row.
  ...
  for (;;) {
    ...
    stepCut = cut === true || (run && run.cut === true);
    const parsed = parseEnvelope(run ? run.stdout : "");
    // The price of THIS call. A retry that came back unpriced must not inherit the first attempt's
    // cost into its own row: two rows, one price each, or the bench sums a call twice.
    ...
    // #7226: the attempt about to be retried was a CLI call of its own — often a boxed one, since a
    // cut turn is no output and no output is malformed — so it gets its own run row HERE, before the
    // retry's call. Written now, a runner that dies during the retry cannot fold it into the step's
    // row, and §8.7 counts every turn the box actually ran instead of one per step.
    recordStep(D, project, card, {
      turn: state.cursor.turn, rev: null, by: seat, ts: now, exit, cost, cut: stepCut, disturbed,
      action: null, verify: null, verified_paths: [], rejected: rejection, retry: true,
    });
    attempt++;
...
/** One run row for `~/.agent-bus/state/runs/<project>-<card>.jsonl` — one per CLI call the step made,
 *  so a retried attempt is a row of its own (#7226). The FIELD NAMES ARE A CONTRACT with
 *  bin/state-bench.mjs (docs/CONTRACT-state.md §run record); a missing cost stays null, never 0. */
export function recordStep(D, project, card, row) {
  ...
  if (row.retry) out.retry = true;   // #7226: the attempt runStep retried in-step; its outcome is the next row
