...
// Trantor State P6 — the runner-side driver (TDD §4.1, §4.8, §7.3).
//
// bin/crew-runner.mjs is not importable: it reads process.argv and awaits enrolment at module
// scope, so anything living inside it can only be proven by standing up a hub. The repo has met
// this before and answered it the same way — lib/classify-failure.mjs was lifted out of the runner
// precisely so test/crew/test-crew-classify.mjs could hold it — so the step logic lives here and
// the runner keeps the wiring: the flagged CLI row, the spawn, and the loop.
//
// WHAT THIS FILE IS FOR, in one line: it is the ORDER. §4.1 is not a suggestion —
//
//   readState → assemble → run the CLI → tier 1 (git touch + credit expiry) → applyTurn
//     → commit (CAS) → promote → execute(action)
//
// — and every step of it is recorded in `trace`, so test/state/test-runner-state.mjs asserts the
// order as a fact rather than by reading the source. A rejected patch stops at applyTurn: nothing
// is committed, nothing is promoted, nothing is executed, and the rejection becomes the next
// observation.
...
// The paths and the budgets belong to §7.3/§7.5 and P7 published them. Importing them upward from
// bin/ is a layering inversion and it is the deliberate one: the alternative is a second copy of
// `~/.agent-bus/state/runs/<project>-<card>.jsonl` and of 0.02/0.10 in this file, and the TDD's
// rule is that no number has a second home. state-bench.mjs guards its own dispatch, so importing
// it runs nothing.
...
/**
 * The JSON Schema handed to `claude --json-schema` (§4.6, §7.3's enforced row).
 *
 * Built from LISTS rather than a hand-typed list of the four names: a fifth list added to the
 * schema and not to this object would be a grammar the validator accepts and the CLI cannot emit,
 * which is the quietest possible way for the two halves to drift.
 *
 * `action` is the §4.6 collapse — `{tool,input}` is deliberately absent, because on a CC seat one
 * step is one whole agent loop and the tool variant belongs to the raw-API driver a later phase
 * needs. Emitting it here would let a seat ask for a tool call nothing in this driver runs.
 */
...
/**
 * Whether the installed CLI can enforce the grammar (§6: the minimum version is unconfirmed, so
 * the runner probes rather than assuming). No flag = state mode stays OFF; a seat that cannot be
 * held to the schema must not be run on a path whose budget assumes it is.
...
/**
 * Parse the `--output-format json` envelope into the two things a step needs: the cost struct and
 * the TurnResult.
 *
 * `result` arrives as the structured object on a schema-enforced run and as a JSON string on some
 * releases; both are accepted, and anything else is `turn: null` with a reason — never a guessed
 * empty patch, which would be indistinguishable from a seat that legitimately changed nothing.
 * @returns {{ envelope: object|null, cost: object|null, turn: object|null, error: string }}
 */
...
/**
 * The card log a state step is read against (§4.1's `tail`), decoded from what `GET /tasks` returns.
 *
 * The hub's note rows are a string on older records and `{by, text}` on current ones, so this is a
 * boundary parse with a domain value out the far side: one string, or "" when the card is not on
 * the board. Pure, so test/state/test-runner-state.mjs can hold the shapes without a hub.
 */
...
/**
 * TIER 1 (§4.8), and it is the live half of R12.
 *
 * Two jobs, and skipping the second is the bug that costs a day: git says what changed, so
 * `touched` is set from it; and every path still carrying a credit is RE-HASHED, with the credit
 * dropped wherever the blob sha has moved off `files[p].hash` or the file is gone. Setting
 * evidence is the gate's privilege. EXPIRING it is tier 1's, because tier 1 is the only thing that
 * runs every single turn — and without this half `verified` goes monotonic-true in production
 * while every unit test still passes.
 *
 * Never sets `verified: true`. Touching a file is not evidence that it works.
...
/**
 * §7.3's circuit breaker, as a verdict over the recorded patch outcomes.
 *
 * Counts MALFORMED output only. `UNVERIFIED_DONE` and `NEEDS_GATE` are the system working — a seat
 * writing perfect patches against a failing test is the healthiest seat there is, and counting its
 * rejections would drop it back to the transcript path exactly when the evidence loop is doing its
 * job.
 *
 * The window must be FULL before it can trip. A rate of 1.0 measured over one turn is not a rolling
 * rate, and a breaker that fires on a seat's first fumble would disable the path this phase exists
 * to measure before it had a sample.
 * @param {{seat?:string, class?:string, code?:string|null, retry_code?:string|null}[]} records
 */
...
/**
 * ONE STEP, in the §4.1 order.
 *
 * `deps` exists so the order can be asserted rather than described: a test injects fakes for every
 * side effect and reads `trace` back. Defaults are the real modules.
 *
 * deps.callCli(prompt, { attempt }) -> { exit, stdout }
 * deps.executeAction(action, ctx)   -> anything; runs LAST, after promote, and only on an accepted
 *                                     patch — "promote before act" is the half of §4.1 that is
 *                                     easiest to lose in a refactor because nothing downstream
 *                                     notices for a while.
 *
 * @returns {{ ok: boolean, trace: string[], code: string|null, message: string, step: object,
 *             state: object|null, action: object|null, observation: string, promoted: string|undefined,
 *             patchRecord: object }}
 */
...
/**
 * One run-step row, appended to `~/.agent-bus/state/runs/<project>-<card>.jsonl`.
 *
 * The FIELD NAMES ARE A CONTRACT with bin/state-bench.mjs and are not free to rename: gate 3 reads
 * `cache_read`, gate 4 reads `cost_usd`/`input`/`output`, gate 5 reads `disturbed`/`action`/`input`,
 * gate 6 reads `rev`/`verify.cmd`/`verify.exit`/`verified_paths`/`rejected.code`/`by`, and §8.7
 * reads `cut`. A row written with the wrong names is a run the bench cannot see, which reads as
 * NO_RUN — the failure this package exists to end.
 *
