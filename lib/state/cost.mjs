...
// Trantor State P5 — cost (TDD §4.6, §4.8): the one place stream-json is legitimate, as the COST source
// only. It imports no state and owns no writes, so it can never set `verified`.
 ...
 *  object returns null: an unknown cost is null, never 0, because a fake 0 flatters the ≥5× gate.
...
/** Sum a CC transcript's assistant usage rows into the same CostStruct; `cost_usd` stays null when
 *  rows carry no `costUSD`. A missing file or no usage rows is null, same rule as the envelope.
 *  @param {string} path
 *  @returns {CostStruct|null} */
