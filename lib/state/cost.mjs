...
// Trantor State P5: cost (TDD §4.6, §4.8). The one place stream-json is legitimate: the stream is
// the COST source, never an EVIDENCE source, and this module imports no state and owns no writes
// so the rule holds mechanically. Two sources, one struct: fromEnvelope (--output-format json)
// and fromTranscript (the whole CC transcript's assistant usage rows).
...
/**
 * Parse one `claude -p --output-format json` envelope into a CostStruct. Anything without a usage
 * object returns null: an unknown cost is null, never 0 (a fake 0 flatters the ≥5× gate).
 */
...
/**
 * Sum a CC transcript's assistant usage rows into the same CostStruct. Rows without costUSD leave
 * `cost_usd` null rather than inventing a price; a missing or empty file is null.
 */
