 ...
 *
 * IT READS `ctx` FIRST, AND THAT IS THE WHOLE POINT — found by P6 when the wiring was finally
 * built (#6969). The gate's facts arrive in `ctx` and are only written into the state by apply.mjs
 * stage 5, which never runs when validate rejects first. So reading `state` alone meant a GREEN
 * gate looked exactly like a red one on the second call, and `applyTurn` answered a green suite
 * with "the gate ran and did not pass (suite, exit 0)" — a message contradicting itself in its own
 * text. The §4.8 cure could never succeed, which left the evidence pipeline dead on the live path
 * while all 264 Phase-0 assertions stayed green: nothing had ever fed a green ctx to a done-move.
 *
 * The precedence is deliberately the same one apply.mjs stage 5 uses — `verify` taken wholesale
 * from ctx when present, `files` merged per path — so the check and the write stay ONE rule rather
