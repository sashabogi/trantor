...
/** Pull the evidence marker out of an item's text BEFORE any cap, merged with `paths` the patch supplied
 *  (TDD §3). Extract, then cap: a long text can never truncate away the paths route (b) needs.
...
/** Does this item have the evidence a `move → done` needs (TDD §4.2)? Route (a): the gate went green this
 *  turn; route (b): every named path is credited. Reads `ctx` FIRST (#6969), with apply.mjs stage 5's precedence. */
