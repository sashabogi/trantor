...
// Trantor State: apply (TDD §4.2, stages 4-5). Validate, apply to a CLONE, runtime pass. Pure: `ctx`
// carries the clock and the harness facts. One apply point per turn is the crash-safety argument
// (§4.4): a killed turn has never half-applied a patch. docs/CONTRACT-state.md.
...
/**
 * The five-stage pipeline. `turn` is the TurnResult `{ patch, action }`; ctx = { now, by, verify,
 * files, gate_attempted, gate }: harness facts, the NEEDS_GATE/UNVERIFIED_DONE split (§4.8), the memo.
 */
 ...
 * `in_flight`/`next` are scratch and promote nothing (#6669). @returns {{ kind, text }[]}
 */
