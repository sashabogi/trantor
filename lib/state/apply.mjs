...
// Trantor State — apply (TDD §4.2, stages 4-5): validate, apply to a CLONE, then the runtime pass. Pure.
// One apply point per turn is the crash-safety argument (§4.4): a killed turn never half-applied a patch.
...
/** The five-stage pipeline over `turn` = { patch, action }. ctx = { now, by, verify, files, gate_attempted,
 *  gate } carries harness facts from a gate that actually ran, never testimony (docs/CONTRACT-state.md).
 *  @returns {{ ok: true, state: object, promoted: object[] } | { ok: false, code: string, at: string, message: string }} */
...
/** The delta the board may see (TDD §4.7): what shipped, what blocks, what the evidence says.
 *  `in_flight`/`next` are scratch and promote nothing (#6669). @returns {{ kind: string, text: string }[]} */
