...
// Trantor State: the wire contract (TDD §3). One place for the shapes, caps, write matrix and the
// evidence marker; validate.mjs and apply.mjs hold behaviour. Nothing here reads disk or clock.
...
/** @typedef {{ schema_version: 3, card: number, task: string, done: Item[], in_flight: Item[],
 *   next: Item[], blockers: Item[], done_count: number, files_count: number, files: Record<string, FileFact>,
 *   verify: { built?: boolean, tested?: boolean, observed?: boolean, cmd?: string, exit?: number },
 *   notes: string, ext: Record<string, unknown>, cursor: { turn: number, ts: number, by: string }, rev: number }} WorkingState */
...
/**
 * The evidence marker, made structural (TDD §3): "... @lib/state/promote.mjs,test/state/test-promote.mjs"
 * is extracted into `paths` BEFORE any cap. This regex lives here and nowhere else.
 */
...
/**
 * Shape-check a whole WorkingState. Deliberately total: never throws, returns the first problem
 * as a string, "" means valid.
 */
