/* oxlint-disable anti-slop/no-runtime-typeof -- SAFETY: state, promotions and the hub response all arrive from outside this module, so the typeof/Number.isInteger guards below ARE the boundary decode. */
// Trantor State P3 — the promoter (TDD §4.7): one turn's delta → AT MOST ONE card note, deduped by hash,
// ext._promoted advanced only on a 2xx. Never moves status, never promotes scratch (#6669).
...
/** Promote one turn's delta to the card. Pure except for the one POST; `post` is injectable.
 *  @param {object} state  @param {object[]} promotions  applyTurn's `promoted` plan
 *  @param {{ by?: string, project?: string, post?: Function, timeoutMs?: number }} [opts]
 *  @returns {Promise<{ok:boolean, sent:boolean, skipped?:string, note:string, hash:string, state:object, status?:number, error?:string}>} */
