// Trantor State P5.5 — the gate (TDD §4.8). runGate computes and returns; it never touches state.
// Its three invariants (content-keyed memo, scoped before scripts.test, R11) are in docs/CONTRACT-state.md.
...
/** Command resolution (§4.8), first match wins: TRANTOR_STATE_GATE → the SCOPED `test/run.mjs --only
 *  <subsystem>` when every path sits under one suite-backed subsystem → scripts.test → none. Pure. */
...
/** Tree CONTENT hash: HEAD sha + a worktree tree sha from a SCRATCH index (`git add -A` folds untracked
 *  files in), so the seat's real `.git/index` is never touched. Scratch dir: `<cwd>/.agent-bus-out/`. */
export function contentHash(cwd) {
  ...
  // The scratch dir must be gitignored or `add -A` folds the index's own bytes in and the memo never
  // hits; `.git/info/exclude` is repo-local and worktree-safe. No pathspec: git refuses `:(exclude)` here.
  if (!outside) {
...
export function touchedPaths(cwd) {
  // -z, and it is not a style preference: without it git C-quotes any unusual path (`caf\303\251.mjs`),
  // the credit a seat names never matches its file, and route (b) fails closed silently.
...
/** Run the gate for the `{ items, paths }` NEEDS_GATE carried. opts: cwd, env, memo (prior ext._gate),
 *  maxMs (a timeout is a RED gate, exit 124). Return shape and credit rules: docs/CONTRACT-state.md §gate.
 *  A RED gate credits no path; the NEEDS_GATE/UNVERIFIED_DONE split belongs to the core. */
