// trantor cryptographic identity: THE INTERFACE CONTRACT for Phase 0 (docs/TDD-trantor-platform.md
// §7); shapes are frozen. An identity is a KEYPAIR and the label is cosmetic, because a
// self-asserted `from` once turned delivery into remote code execution. Ed25519 via node:crypto,
// every primitive behind this module, no Nostr interop (stated in the TDD).
...
// A per-instance keypair ENDORSED by the durable identity: the durable key keeps enrollment/grants/
// attribution; the instance key signs traffic and dies with the session (handoff-twin fix).
