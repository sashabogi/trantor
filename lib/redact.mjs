// #5869: key-material redaction for everything a seat runner writes at rest (any seat can read
// ~/.agent-bus). Known key shapes become `<redacted:NAME>`: sk-*, AIza, xai-, ghp_, <VAR>_KEY= /
// <VAR>_TOKEN= with a 32+-char value, Authorization: Bearer. Rules anchor on a prefix or position,
// never on "looks long"; ordinary lines are byte-identical; idempotent. docs/CONTRACT-lib.md.
const RULES = [
...
// The runner's tee replacement: --tee <file> (stdin → stdout verbatim, redacted → file) and --tee2
// (stdin → stderr verbatim). The redacted append is LINE-BUFFERED because a chunk can split a
// match mid-token but keys never span lines; every byte is appended exactly once, in order.
