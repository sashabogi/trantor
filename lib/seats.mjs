...
// Identity is positional, and a reboot moves every window to $HOME, so this makes the non-seat
// state RECOVERABLE: the operator declares once, and one command (or a login agent) puts every
// missing seat back in its directory. Nothing here guesses. docs/CONTRACT-lib.md.
...
export function readSeats() {
  try {
    ...
    for (const [p, v] of Object.entries(s)) {
      ...
      const agent = (typeof v === "object" && typeof v?.agent === "string" && v.agent) ? v.agent : "claude";
      if (typeof dir === "string" && dir) out[p] = { dir, agent };
    }
    return out;
  } catch { return {}; }
}
...
export function declareSeat(project, dir, agent = "claude") {
  ...
  cfg.seats = { ...(cfg.seats && typeof cfg.seats === "object" ? cfg.seats : {}), [project]: { dir: abs, agent } };
...
// ── liveness ────────────────────────────────────────────────────────────────────
// A seat is live when a real agent process is STANDING IN ITS DIRECTORY, not when the hub's
// presence list says so (a name is assertable, a cwd is not). `ps` gives candidate pids, lsof
// resolves each cwd; both bounded and fail-soft (this runs in a CLI and a login agent).
  ...
  return Object.entries(seats).map(([project, { dir, agent }]) => {
    ...
    return {
      ...
      why: holder ? ""
        ...
        : others.length ? `no ${agent} here (${others.map(o => `${o.comm} ${o.pid}`).join(", ")} is, but that is not this seat)`
...
export function launchSeat(seat, { command, dryRun = false } = {}) {
  const cmd = `cd ${JSON.stringify(seat.dir)} && ${command || seat.agent || "claude"}`;
