...
// Each adapter: { provider, label, kind, envKeys:[...], async fetch(key) -> normalized fields }.
...
export const ADAPTERS = [
  ...
  {
    provider: "moonshot", label: "Moonshot", kind: "prepaid", envKeys: ["MOONSHOT_API_KEY"],
    async fetch(key) {
  ...
  {
    provider: "kimi", label: "Kimi Code", kind: "quota", envKeys: ["KIMI_API_KEY"],
    async fetch(key) {
  ...
  {
    provider: "zai", label: "Z.ai (GLM)", kind: "quota", envKeys: ["ZAI_API_KEY", "GLM_API_KEY"],
    async fetch(key) {
...
// fetch credit for every adapter whose key is present in `env`; adapters with no key are skipped.
