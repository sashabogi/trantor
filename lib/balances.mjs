// trantor provider credit — read-only "how much is left" per provider, keys from the environment.
// Two kinds of left: prepaid (a balance) and quota (a window that resets); docs/CONTRACT-lib.md §balances.
...
// Adapter shape: { provider, label, kind, match:[profile names], envKeys, async fetch(key) }. Balances
// are queried only for providers in the user's profile, never for every key in the ambient env.
export const ADAPTERS = [
  {
    ...
    keyless: true,
    async fetch() {
      // USAGE v2: the statusline sidechannel keeps a live local cache; when the OAuth endpoint fails
      // (it 429s under polling, docs/RESEARCH-orca-usage.md §1.1) a fresh capture answers instead.
      const live = (() => {
        try {
      ...
      const win = (w, name) => (w && w.utilization != null)
        ...
        : null;
      // Model-scoped weekly limits ride limits[] as their own segment ("36% used Fable"); the
      // session/weekly_all entries there duplicate five_hour/seven_day, so only scoped ones add information.
  ...
  {
    // #6131: the Qwen plan's gauge is cookie-only, so this reads the WALL (a 429 insufficient_quota
    // with the reset time) and says "unknown" otherwise; the empty-messages probe costs no tokens.
...
export async function fetchBalances(env = process.env, opts = {}) {
  ...
  // Codex: no public balance API, so the CLI's own token reads the ChatGPT usage windows (#5570);
  // unreachable → the flat subscription row. Profile-gated like every adapter: no profile, no row.
...
export function fmtBalance(e) {
  ...
  if (e.kind === "quota") {
    // #7413 — a missing number with an adapter detail (qwen's active plan) prints the detail, so
    // the CLI line and the app chip agree; "quota unknown" is for a gauge nobody could read.
    if (e.remainingPct == null) return `${e.label}${e.plan ? " (" + e.plan + ")" : ""}: ${e.detail || "quota unknown"}`;
