...
|---|---|---|
| codex | `lib/overseer.mjs`, `test-overseer-lib.mjs` | Pure module, NO I/O, NO imports beyond node core. Export `detectCollisions({ peers, claims, links, cards, autonomy, now }) -> collision[]` and `levelFor(project, autonomy) -> number` (project key, else "*", else 1). Inputs: peers = [{session, project, lastSeen, llm, model, status}] (live = lastSeen within 5m of now); claims = [{project, file, session, ts}] (a claim's `project` is the CLAIMANT's, not the file's); cards = [{id, project, status, assignee, workedBy}], open cards only; links/autonomy per shapes above. Kinds: (1) same-project-sessions — ≥2 live sessions on one project → one collision per project listing them; (2) file-conflict — ≥2 live claims by different sessions on same (project,file) within 10m → one collision per file; (3) linked-activity — an EVENT spanning ≥2 projects of one declared link → one collision per link. **Presence is not an event** (#7029): a link is DECLARED, so two sessions being live on both sides restates the operator's own declaration and, where the codependence is permanent, is permanently true — it woke seats to prove negatives (468 identical warnings in 8 days on crebral, 2026-08-12). Two events count, both already in hub state: one FILE PATH claimed live from both sides — note that kind (2) keys on (project, file) and therefore **cannot** see a cross-project overlap, which is why this kind must not fall back to presence to cover it — or one CARD ID in `doing`/`testing` whose holders (`assignee` = intent, `workedBy` = the signed evidence of who moved it) are live sessions in two different projects of the link. No event, no collision; `files` carries the overlapping paths and `detail` cites the evidence. Deterministic ordering (sort by project, kind, first file/session). Unit tests: every kind, level fallback, empty inputs, dedup, ordering. Test runs with plain `node test-overseer-lib.mjs`, prints ✓/✗ lines, exits non-zero on failure (house style: read test-claims.mjs). |
