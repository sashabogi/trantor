//! Ghost-text fast path (#5897): ONE direct OpenAI-compatible call from the app process to a fast
//! model (qwen preferred, deepseek fallback), config read at RUNTIME from ~/.agent-bus/.env.
//! Small on purpose: max_tokens 32, stop at the first blank line, 2 s timeout so a slow provider
//! degrades to "no ghost". Request shaping is pure and unit-tested; only the round-trip is not.
...
// The target is TIME-TO-FIRST-LINE: read the SSE stream as it arrives, forward each delta on a
// request-keyed channel, stop at the first complete line (or ~32 tokens) and abort the rest.
// The 2 s budget applies to first-line arrival.
...
/// the only per-keystroke part (the last few lines before the cursor) LAST, so keystrokes N and
/// N+1 share a prefix and prefill can hit the cache.
...
/// request, forwards each delta on `on_event`, resolves at the first complete line (or ~32 tokens)
/// and aborts the rest. The 2 s budget is TIME-TO-FIRST-LINE; past it, no ghost.
#[tauri::command]
...
mod tests {
    ...
    // ── #5897 latency probe: NOT part of `cargo test` (80 REAL calls, spends the key). Run it on
    // purpose: cargo test ghost_latency_matrix -- --ignored --nocapture. Same request builder as
    // ghost_complete, max_tokens 32/64 × 30/60 lines, prints p50/p95 + a sample first line.
    const RUNS: usize = 20;
    ...
    // ── #6160 streaming probe: p50 TIME-TO-FIRST-LINE under 600 ms is the acceptance item. Same
    // 20-run harness, but streamed: t0 → the first '\n' of completion content. Spends the key:
    // cargo test ghost_stream_latency -- --ignored --nocapture. 15 s ceiling so slow calls are RECORDED.
