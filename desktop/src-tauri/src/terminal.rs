...
/// The one boot prompt a WOKEN session gets. Same doctrine as handoff_now's KICKOFF_PROMPT (a
/// session never runs a turn unprompted), but wake must not assert a handoff exists.
...
pub fn orchestrator_open(project: String) -> Result<String, String> {
    ...
    // `trantor open` inherits the caller's cwd, and the app's cwd is nowhere near the checkout, so
    // claude once booted in the wrong folder (trust prompt, wrong transcript slug, no memory).
    // Resolve the dir first, as handoff_now always did.
    ...
    // KICKOFF-AFTER-WAKE: only when this open actually STARTED a conversation. A reattach ("already
    // hosted: reattached", the phrase crew.sh open_orchestrator prints) is someone's live session.
    // The waiter is a plain thread; herdr::prompt refuses Blocked/NotReady/NoAgent BEFORE any bytes
    // land, so retrying is safe. Stalled means bytes may have landed: never retry past it.
...
mod tests {
    ...
    /// The headless half of #6094's real-path bounce: writes the EXACT byte sequences
    /// answerKeystrokes()/submitOther() build (single pick, multi-select, "Other") through the SAME
    /// `TerminalManager::write()` the real IPC command calls, into a real pty running an echo child.
    /// Mangled here = this crate's write path; passing here = the bug is in herdr's attach relay.
    #[test]
