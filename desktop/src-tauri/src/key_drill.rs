//! #6317: the Objective-C exception reporter and the built-app key-injection drill. SIGABRTs from
//! tao's `sendEvent:` override logged no first panic: an AppKit exception unwound through the
//! `extern "C"` frame. The vendored tao now catches it there and calls the reporter installed here.
//! `TRANTOR_KEY_DRILL=post|throw` (+ `TRANTOR_KEY_DRILL_PROJECT`): docs/CONTRACT-desktop-shell.md.
