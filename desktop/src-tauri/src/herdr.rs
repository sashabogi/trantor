...
//! "prompt delivery" + "agent lifecycle"; docs/CONTRACT-desktop-shell.md). Sends ride `agent.prompt`
//! (herdr owns paste mode, Enter encoding, and refuses a blocked agent BEFORE any bytes land).
//! Socket, never CLI (a leading "-" breaks argv); one request per connection.
...
/// Same, with the caller's own read budget. Hot-path QUERIES use a short one: a load-slowed server
/// once held one agent.get for the full default and stalled the transcript tail.
...
/// blocked-refusal is the wrong behavior because the picker IS the blocked state. `pane.send_text`
/// has no agent-lifecycle gating (verified live: escape bytes arrive verbatim, unknown agent ok).
/// Never `agent attach`: a watch client is read-only by design and writing into it returned EIO.
...
/// A live `pane.agent_status_changed` subscription for ONE pane (Phase 3: the composer gate stops
/// polling). Ack: `{"result":{"type":"subscription_started"}}`; frames carry data.{agent,
/// agent_status,pane_id,workspace_id}. Per-pane subscriptions are LIVE-ONLY, unlike the global
/// pane.* types which replay history; hence per pane, re-seeded via `agent_status()`.
...
mod tests {
    ...
    // Captured from a real socket reply (docs/RESEARCH-herdr-prompt.md): CLI and socket share the
    ...
    // Captured live over the real socket against a throwaway pane: the success shape, and a
    // `pane_not_found` error in the same envelope `agent.prompt` uses.
    ...
    // Captured live (scratchpad/status-events.txt): a per-pane subscription frame, Claude Code's
    // startup trust dialog recognized as blocked.
