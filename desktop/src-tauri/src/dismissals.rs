// Interrupted-session dismissals, in ~/.agent-bus/config.json under "dismissedSessions" (#6476).
// A dismissal is a decision, not a snooze, so it survives a restart; keyed on (project, sessionId)
// so a NEW dead session for the same project still shows.
...
mod tests {
    ...
    /// The real path against actual disk: dismiss two sessions, re-read from scratch, both stay;
    /// clearing one project removes only that row. Holds BUS_DIR_TEST_LOCK like right_panel/onboarding.
    #[test]
