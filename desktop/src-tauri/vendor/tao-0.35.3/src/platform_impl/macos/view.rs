...
impl ViewState {
  fn get_scale_factor(&self) -> f64 {
    // #6317: `ns_window` is a weak ref and can already be gone once AppKit starts tearing the
    // window down while this view still receives events; 1.0 is the same fallback tao's own
    // `MonitorHandle::default()` scale factor uses, not a magic number invented here.
    self
      .ns_window
      .load()
      .map(|w| (unsafe { NSWindow::backingScaleFactor(&w) }) as f64)
      .unwrap_or(1.0)
  }
}

/// #6317: every event handler below used to `.unwrap()` this weak ref, panicking inside an
/// `extern "C"` AppKit callback with no unwind ABI (`panic_cannot_unwind` -> SIGABRT) whenever
/// the window had already started tearing down but the view kept receiving late events (e.g. an
/// IME/key event settling after a composer send). A gone window has nothing to report events
/// to, so callers treat `None` as "no-op this callback" instead of crashing the process.
fn live_window(state: &ViewState) -> Option<Retained<NSWindow>> {
  state.ns_window.load()
}

fn live_window_id(state: &ViewState) -> Option<WindowId> {
  live_window(state).map(|w| WindowId(get_window_id(&w)))
}
...
pub unsafe fn set_ime_position(ns_view: &NSView, input_context: id, x: f64, y: f64) {
  ...
  let Some(window) = state.ns_window.load() else {
    return;
  };
...
extern "C" fn draw_rect(this: &Object, _sel: Sel, rect: NSRect) {
  unsafe {
    ...
    if let Some(position) = state.traffic_light_inset {
      if let Some(window) = state.ns_window.load() {
        inset_traffic_lights(&window, position);
      }
    }

    if let Some(window_id) = live_window_id(state) {
      AppState::handle_redraw(window_id);
    }
...
extern "C" fn insert_text(
  ...
  unsafe {
    ...
    let Some(window_id) = live_window_id(state) else {
      trace!("Completed `insertText` (window already gone)");
      return;
    };
...
extern "C" fn key_down(this: &mut Object, _sel: Sel, event: &NSEvent) {
  ...
  unsafe {
    ...
    let state = &mut *(state_ptr as *mut ViewState);
    let Some(window_id) = live_window_id(state) else {
      trace!("Completed `keyDown` (window already gone)");
      return;
    };
...
extern "C" fn flags_changed(this: &Object, _sel: Sel, ns_event: &NSEvent) {
  ...
  unsafe {
    ...
    let state = &mut *(state_ptr as *mut ViewState);
    let Some(window_id) = live_window_id(state) else {
      trace!("Completed `flagsChanged` (window already gone)");
      return;
    };
...
#[cfg(test)]
mod window_teardown_tests {
  // #6317: the 2026-09-03 crashes (10:08, 21:48) were this exact shape — a key/IME event
  // landing on a `TaoView` whose window had already started tearing down, so `ns_window.load()`
  // returned `None` and the old `.unwrap()` calls turned that into a SIGABRT. `Weak::default()`
  // reproduces "window already gone" without needing a real `NSWindow` or the main thread, so
  // this runs as an ordinary `cargo test`.
  use super::*;

  fn state_with_gone_window() -> ViewState {
    ViewState {
      ns_window: objc2::rc::Weak::default(),
  ...
  #[test]
  fn get_scale_factor_falls_back_instead_of_panicking_when_the_window_is_gone() {
    let state = state_with_gone_window();
    assert_eq!(state.get_scale_factor(), 1.0);
  }

  #[test]
  fn live_window_helpers_return_none_instead_of_panicking_when_the_window_is_gone() {
    let state = state_with_gone_window();
    assert!(live_window(&state).is_none());
    assert!(live_window_id(&state).is_none());
  }
}
