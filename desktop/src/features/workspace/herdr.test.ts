...
// #6094 real-path bounce: answerAtPane's FIRST version wrote into its own `herdr agent attach` (a
// read-only streaming watch client) and always failed with EIO. The fix routes through the Rust
// `ask_answer` command (`pane.send_text`). This asserts the NEW writable command is called, using
// the same injectable-invoke seam Chat's ChatDeps uses rather than a mocked module.
