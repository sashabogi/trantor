// The ONE right mode pane (#5841, .scratch/design-v4/*.dc.html): Files | Git | Sessions | Chat. It
// REPLACES the chat dock, the left tree column and the SCM rail by MOVING their pieces here. Widths
// follow the artboards (300 / 440 for Chat); the seat scope selector at the bottom feeds tree,
// editor and git alike.
...
export function ModePane({ client, project, seat, onSeat, onOpenFile }: {
  ...
  // #6499: the panel remembers its tab per project (config.json via rightPanelState.ts); an untouched
  // project defaults to Chat while its orchestrator is live, Files otherwise. One resolve per
  // project, not a poll, so a live orchestrator never yanks the panel out from under a choice.
  useEffect(() => {
    let alive = true;
    void (async () => {
  ...
  // The tab strip's measured layout (#6036): a tab word NEVER truncates. The strip compares the
  // natural width of its four labels (read off twin buttons INSIDE the strip, same cascade) with
  // the width it has, and goes icon-only when they would not fit. Unmeasured stays on labels.
  ...
  // Changed-file count per seat, for the footer chips ("which worktree has edits"). Absent = clean.
  const seats = useMemo(
  ...
  // The rail is the SAME object as the lens tabs (tr-seg) with a Lucide icon per mode. Segments share
  // the strip's width (#5960). A label never truncates (#6036): when the measured labels would not
  // fit, the strip steps down to icon-only; the truncate class is the last-resort guard.
  ...
  // One truth for the real tabs and the twins: same parent, icon, label, dot and gap. #6499: the
  // Chat tab's needs-you badge rides the live-dot slot, so both measure the same width.
  ...
  return (
    <div
      ...
      style={{ width: paneWidth, margin: 12, maxWidth: `max(${PANE_MIN}px, calc(100vw - ${PANE_ROW_RESERVED}px))` }}
    >
      ...
      />
      {/* mode rail — the segments share the strip's real width and shrink under it (#5960);
          icon-only below the measured label-fit width so a tab word never truncates (#6036).
          The twin tabs render next to the real ones as DIRECT children — same parent, so the
      ...
      <div ref={stripRef} className="tr-seg relative grid shrink-0 grid-cols-4 gap-px border-b border-tr-edge px-2 py-2">
        {MODES.map(({ m, label, Icon, dot, needsYou }) => (
          <button
            ...
            aria-hidden="true"
            tabIndex={-1}
            className="pointer-events-none invisible absolute top-0 left-0 flex shrink-0 items-center gap-1 whitespace-nowrap rounded-[7px] px-1 py-[5px] text-[11.5px]"
          >
            <Icon size={12} strokeWidth={1.75} className="shrink-0" />
