// The editor's document store (#5938): state that must OUTLIVE the lens (AppShell unmounts the Code
// surface on every lens switch). Files is a VIEW over this store. Composition, not duplication:
// tab operations stay in codeTabs.ts and the conflict decision in tabGuard.ts; this file OWNS the
// maps (tabs, activeKey, per-tab { draft, disk, baseSignature, loaded }).
...
/** Whether a lens may write its local draft back as the tab's kept work (#5938): an active
 *  document, finished loading, and a VIEW that hydrated from that very document. The third leg
 *  once let a remount stash the initial "" over a kept draft. */
