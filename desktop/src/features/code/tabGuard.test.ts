...
// The empty-editor regression: a tab switch stashed the pre-load "" as a kept draft, and a draft
// with NO base signature was read as evidence the disk moved. No signature, no verdict.
...
describe("externalMutationOnLoad", () => {
  ...
  it("yields NO verdict without a baseline — the rule the empty-editor regression rewrote", () => {
    // This leg used to assert the opposite ("still flags"), the semantic that showed an empty editor
    // under a false conflict bar: a pre-load "" stash has no baseline.
