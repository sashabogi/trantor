// Ghost-text debounce + cancel core (#5897), PURE so timing rules are unit-tested without Monaco
// or Tauri. A "latest-wins" gate: schedule() debounces, and a newer keystroke invalidates
// whatever is pending OR already in flight. The caller resolves to [] on a superseded result so
// Monaco never paints a stale ghost; every promise still settles, never a hang.
