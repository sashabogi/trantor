// A real editor: Monaco (#5790), bundled locally via vite workers (monacoSetup.ts). Same props as the
// CodeMirror build: path is the editor's identity, a new value for the same path replaces in
// place, ⌘S saves. No language server (#6437): suggestions are Monaco's built-ins. Ghost text
// (registerGhostTextProvider) is unrelated.
...
export function CodeView({ value, path, editable, onChange, onSave, project, seat }: {
  ...
  useEffect(() => {
    ...
    // Setup is over the moment the editor is up (#5938): this effect re-runs on `path`, and nothing
    // else cleared the guard, so every keystroke was muted. The value effect raises and lowers it.
  ...
  // A new document for the same path replaces the text without tearing the editor down;
  // pushEditOperations keeps the undo stack. One skip (#5857 bounce): while a resumed tab is still
  // loading the prop is "" and the model holds the STORE's draft; pushing "" would erase it.
  useEffect(() => {
