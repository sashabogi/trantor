// The Code lens's tab model (#5813), Orca's preview/pin anatomy (RESEARCH-orca-renderer.md §4):
// identity is scope+path, a plain click opens a PREVIEW the next preview replaces, a pin is
// permanent. Pure so "my pinned tabs never move" and "the dirty dot follows the draft" are tested.
export type CodeTab = {
...
/**
 * Open a path under a scope, preview semantics: an active preview is replaced in place, else a
 * fresh preview opens at the end. An existing tab is activated, never duplicated or moved.
 */
