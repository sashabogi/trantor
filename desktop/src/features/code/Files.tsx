// Reading and changing the code, in the app: the v3 editor core (#5809), shaped by Orca's renderer.
// 1. A file opens EDITABLE, always (the seat-working guard holds server-side via herdr).
// 2. "Changes" is the open file wearing a diff on the SAME draft. 3. Saving is a PLAIN write.
// 4. Open files are TABS (#5813, codeTabs.ts): preview vs pin, dirty dot follows the draft.
...
export function Files({ project, lens, onLens, path, seat }: {
  ...
  // stashes the outgoing tab's draft first, but ONLY a draft whose document finished loading: a
  // draft without a completed load is a loading screen, not a draft.
  const stashDraft = () => {
    if (!activeKey) return;
