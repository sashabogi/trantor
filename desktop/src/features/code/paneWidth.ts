// The mode pane's drag-resizable width (#6086): pure clamp + persistence, per mode. The #6036
// observers watch the pane root, so a live drag re-fits the tab strip. Storage rides the same
// injected Store the chat prefs use; every reader is total, bytes decoded, never trusted.
