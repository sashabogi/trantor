...
// Rust's `project_sessions` (docs/CONTRACT-chat-streaming.md v5): process + filesystem truth, no
// heartbeats, no hub. Branches mirror the CLI chain (`trantor takeover`) exactly, so the button
// can never offer what the chain would refuse.
