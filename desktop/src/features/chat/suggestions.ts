// Suggested-reply chips (#5929): the pure extractor. At most three chip labels read strictly from
// what the orchestrator's last turn says; nothing invented, nothing sent. Adoption is evaluated
// later from `promptSource: "suggestion_accepted"`. #6702: a prose chip carries the sentence it was
// read from (`tooltip` on hover, `ask` as the row's lead-in; they differ for a numbered pick).
...
export function suggestionsFromTurn(text: string): Suggestion[] {
  ...
  // 2. "say <word>", the operator's idiom for "answer with exactly this word"; the orchestrator
  //    confirms in prose too (#5993), so only words that ARE answers count. A plain affirmation
...
/** An AskUserQuestion carries its closing question as structured options, so its options ARE the
 *  chips, verbatim, capped at three. Takes the option shape structurally so this stays the pure,
