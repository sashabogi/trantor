// STATUS ARBITER, pure on purpose (#6146). `status` is fed by two racing sources: a one-shot
// `orchestrator_status` seed and a stream of "orch-status" pushes. A seed that resolved AFTER a
// push once stomped it. The fix is ordering, not timing: every update carries a `seq` assigned
// when DISPATCHED (seed) or RECEIVED (push), and `apply` keeps the highest seq it has seen.
