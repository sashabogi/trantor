// MESSAGES: the fleet's conversations, Slack-shaped. Agent↔agent DMs rendered nowhere before this
// (Chat is per-project, Inbox is what is addressed to the human). Zero new hub machinery: every
// /send is a `message` EVENT; conversations are a client-side GROUPING (a DM thread per session
// pair, a broadcast thread per project). Every DM thread composes, folding in the human's exchanges.
...
/** The name a human recognizes, per session KIND: an interactive window drops its machine head
 * ("crebral-health"), a crew seat keeps the agent ("glm:crebral-health"), the duty seat reads by
 * its job. The full session id stays available as a tooltip. */
...
function msgOf(ev: HubEvent): Msg | null {
  ...
  // hub:* traffic is the overseer reporting STATE, with its own surface (the Overseer view), and
  // rendering it here as fake conversations buried every real exchange. Messages is conversations
  // between parties that can actually converse.
...
// Any surface that renders a log must ROLL IT UP: overseer notices arrive dozens at a time with
// near-identical text. Consecutive synthetic-sender messages collapse into ONE quiet row (count +
// span, expandable); real agents' words always render in full.
