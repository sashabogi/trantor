// genesisFlow.ts: the pure state machine behind the "Start a project" sheet (#6161). The sheet
// used to await BOTH `trantor new` AND the wake, so a slow wake left it open with a dead Cancel.
// Now it closes the moment `trantor new` succeeds and the wake is a detached step with a toast.
// No React, no side effects: only the transition table and two predicates, unit-tested.
