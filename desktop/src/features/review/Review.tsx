...
export function Review({ client, project, lens, onLens }: {
  ...
  // the git rail (#5775, now the SCM panel #5791): hidden by default — the diff is the lens's
  // reason to exist — and a nonce the rail bumps after commit/push so the diff re-pulls instead of
  // lying about landed work. `gitFile` is the SCM file whose diff is open in the monaco editor.
