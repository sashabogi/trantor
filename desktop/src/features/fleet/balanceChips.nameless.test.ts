...
// #6391 real path: the Accounts pane blanked the whole app because chipFrom read `.slice` on an
// undefined name for a usage row without label or provider.
describe("chipFrom never throws over a nameless usage row", () => {
  ...
  for (const [i, nameless] of namelessRows.entries()) {
    it(`nameless row ${i} renders a chip or null, never throws`, () => {
      // SAFETY: the test deliberately feeds partial rows; the function must tolerate them.
      const row = nameless as BalanceRow;
