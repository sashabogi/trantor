// A project's own face in the sidebar: the mark already on disk (Rust `project_icon`), because a list
// of same-looking text rows has nothing for the eye to land on. The rest get a deterministic
// monogram in the Avatar hue family. Agents and humans are CIRCLES, projects are ROUNDED SQUARES.
...
/** "crm-platform" → "CP", "crebral-health" → "CH", "capowerball" → "CA". Two characters, always:
 * initials for hyphenated names (first two LETTERS made half the fleet "CR"), first two letters
 * otherwise. Collisions stay (CSS and crebral-scribe both give "CS"): the hue is derived from the
 * full name, so two "CS" tiles differ in colour, next to their own labels. */
