// Roll-up for overseer history, shared by the Overseer view and Home's Collisions pointer. Before
// episode-based warning (hub 0.17.66) a standing condition re-fired every window, so the log holds
// hundreds of identical rows; rendering one row per event reads as a stuck record. ONE place, because
// it once shipped in two and the surfaces drifted.
